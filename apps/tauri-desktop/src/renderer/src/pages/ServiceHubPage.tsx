...
import { ArrowRightIcon, BoxesIcon, ExternalLinkIcon, Link2Icon, MergeIcon, PencilIcon, PlusIcon, RotateCcwIcon, SearchIcon, SparklesIcon, SplitIcon, Trash2Icon, UnlinkIcon } from 'lucide-react';
...
function ApiEnvironmentPanel({ service, logins, secrets }: { service: ServiceDetail; logins: ApiLoginOption[]; secrets: ApiSecretOption[] }) {
  ...
  useEffect(() => {
    setDraft(null); setEditing(null); setRequestEnvironment(null);
  ...
  const openCreate = (): void => {
    setEditing(null);
    setDraft({ serviceId: service.id, name: '', kind: 'production', origin: 'https://', basePath: null, openapiUrl: null, auth: defaultAuth(), fixedHeaders: [] });
  };
  ...
  const persist = (input: ApiEnvironmentInput & { id?: string }): void => { void run(async () => {
    ...
    setDraft(null); setEditing(null); await load(); toast.success('API 环境已保存。');
  }); };
  const save = (): void => {
    if (!draft) return;
  ...
  return <Card>
    ...
    </div>}<DialogFooter><Button variant="outline" onClick={() => { setDraft(null); setEditing(null); }}>取消</Button><Button disabled={busy || !draft?.name.trim() || !draft.origin.trim()} onClick={save}>保存环境</Button></DialogFooter></DialogContent></Dialog>
...
export function ServiceHubPage() {
  ...
  const openCreate = (): void => { setEditingId(null); setDraft(emptyDraft); setEditorOpen(true); };
  const openEdit = (): void => {
    if (!detail) return;
  ...
  const save = (): void => { void run(async () => {
    ...
    setEditorOpen(false); setSplitRelationship(null); await load(id); toast.success(editingId ? '网站/服务已更新。' : '网站/服务已创建。');
  }); };
    ...
    <section className="mx-auto grid w-full max-w-6xl gap-6 px-5 py-8 lg:grid-cols-[20rem_minmax(0,1fr)]">
      ...
      <div className="min-w-0 space-y-5">
        ...
        {!detail ? <Empty className="min-h-96 rounded-2xl border"><EmptyHeader><EmptyMedia variant="icon"><BoxesIcon /></EmptyMedia><EmptyTitle>按网站或服务查看保险库</EmptyTitle><EmptyDescription>创建一个网站/服务，或先预览自动整理。原登录、密钥和 SSH 项目不会被改写。</EmptyDescription></EmptyHeader><EmptyContent><Button onClick={openCreate}><PlusIcon />新建网站/服务</Button></EmptyContent></Empty> : <>
          ...
          <ApiEnvironmentPanel service={detail} logins={apiLoginOptions} secrets={apiSecretOptions} />
