...
export function PopupApp({ initialWorkspace = null }: { initialWorkspace?: InitialPopupWorkspace | null }) {
  ...
  const [items, setItems] = useState<VaultItem[]>(() => initialWorkspace ? toVaultItems(initialWorkspace.snapshot, initialWorkspace.activeUrl) : []);
    ...
    try {
      ...
      if (options.preferCached) {
        ...
        if (cached) {
          ...
          setItems(toVaultItems(cached.snapshot, cached.activeUrl));
      ...
      setItems(toVaultItems(result.snapshot, result.activeUrl));
  ...
  useEffect(() => {
    let active = true;
    if (new URLSearchParams(window.location.search).get("tab") !== "generator") void refreshWorkspace({ preferCached: !initialWorkspace });
    ...
    return () => { active = false; setItems([]); setSelectedIds(new Set()); };
  }, []);
  ...
  const suggestedItems = filteredItems.filter((item) => item.suggested);
  ...
  async function copyItem(item: VaultItem, option: string) {
    try {
      const masterPassword = item.masterPasswordReprompt ? window.prompt("请输入主密码以继续") ?? undefined : undefined;
      const operation = item.type === "登录" ? option === "用户名" ? "items.copy-username" : option === "验证码" ? "items.copy-totp" : "items.copy-password" : item.type === "支付卡" ? option === "卡号" ? "cards.copy-number" : option === "安全码" ? "cards.copy-security-code" : "cards.copy-pin" : item.type === "机密" ? "secrets.copy-value" : option === "公钥" ? "ssh.copy-public-key" : "ssh.copy-private-key";
      await desktopRpc(operation, { id: item.id, masterPassword: masterPassword ?? null }); setNotice("已复制，桌面端将自动清除剪贴板。", "success");
          ...
          {loading && desktopState !== "ready" ? <VaultWorkspaceSkeleton /> : <ScrollArea className="-mr-3 min-h-0 flex-1">
            <div className="flex flex-col gap-4 pr-4">
              ...
              <section className="flex flex-col gap-2">
                ...
                {suggestedItems.map((item) => <VaultRow key={item.id} item={item} showFill fillState={fillState} onEdit={() => setEditingItem(item)} onFill={() => void requestFill(item)} onCopy={(option) => void copyItem(item, option)} />)}
                {suggestedItems.length === 0 ? <EmptyList>当前筛选条件下没有自动填充建议。</EmptyList> : null}
                <FillNotice state={fillState} outcome={fillOutcome} />
              </section>

              <section className="flex flex-col gap-2">
                <div className="flex items-center justify-between gap-2">
                  <ListHeading title="所有项目" count={filteredItems.length} />
                  <Button size="sm" variant="ghost" type="button" onClick={toggleSelectAll} disabled={filteredItems.length === 0}><CheckIcon data-icon="inline-start" aria-hidden="true" />{allFilteredSelected ? "取消全选" : "全选"}</Button>
                </div>
                {selectedIds.size > 0 ? (
                ...
                {filteredItems.map((item) => <VaultRow key={item.id} item={item} selected={selectedIds.has(item.id)} selectable showFill fillState={fillState} onToggle={() => toggleSelection(item.id)} onEdit={() => setEditingItem(item)} onFill={() => void requestFill(item)} onCopy={(option) => void copyItem(item, option)} />)}
                {filteredItems.length === 0 ? <EmptyList>{desktopState === "ready" ? "没有匹配的密码库项目。" : "连接桌面端并在插件中解锁后即可查看保险库。"}</EmptyList> : null}
              </section>
            </div>
          </ScrollArea>}
          </>
        ) : activeTab === "generator" ? <GeneratorPanel onNotice={setNotice} /> : <SettingsPanel fillHistory={fillHistory} onNotice={setNotice} onLocked={() => { setActiveTab("vault"); void refreshWorkspace(); }} />}
      </section>
...
function toVaultItems(snapshot: WorkspaceSnapshot, activeUrl?: string): VaultItem[] {
  ...
  return [
    ...
    ...snapshot.identities.map((item) => ({ id: item.id, title: item.title, detail: item.displayName || item.organization || "身份信息", type: "身份" as const })),
    ...snapshot.cards.map((item) => ({ id: item.id, title: item.title, detail: `${item.cardholderName} · ${item.maskedNumber}`, type: "支付卡" as const, masterPasswordReprompt: item.masterPasswordReprompt })),
    ...snapshot.sshCredentials.map((item) => ({ id: item.id, title: item.title, detail: item.host ? `${item.username}@${item.host}` : item.username || "SSH 凭据", type: "SSH" as const, suggested: hostMatches(item.host, activeUrl), masterPasswordReprompt: item.masterPasswordReprompt })),
    ...snapshot.secrets.filter((item) => !item.isPasskey).map((item) => ({ id: item.id, title: item.title, detail: [secretSummaryLabel(item.kind), item.provider, item.account, item.environment].filter(Boolean).join(" · "), type: "机密" as const, suggested: sameSite(item.website, activeUrl), masterPasswordReprompt: item.masterPasswordReprompt })),
  ];
}
...
function sameSite(itemUrl: string | null, activeUrl?: string) {
  if (!itemUrl || !activeUrl) return false;
  try { const item = new URL(itemUrl); const active = new URL(activeUrl); return item.protocol === active.protocol && item.hostname === active.hostname; } catch { return false; }
}

function hostMatches(host: string | null, activeUrl?: string) {
  if (!host || !activeUrl) return false;
  try { return host.toLocaleLowerCase() === new URL(activeUrl).hostname.toLocaleLowerCase(); } catch { return false; }
}
...
function VaultRow({ item, selectable = false, selected = false, showFill = false, fillState, onToggle, onEdit, onFill, onCopy }: { item: VaultItem; selectable?: boolean; selected?: boolean; showFill?: boolean; fillState: FillState; onToggle?: () => void; onEdit: () => void; onFill: () => void; onCopy: (option: string) => void }) {
  const Icon = item.passkeyOnly ? FingerprintIcon : item.type === "登录" ? KeyRoundIcon : item.type === "身份" ? FolderKeyIcon : item.type === "支付卡" ? CreditCardIcon : item.type === "机密" ? BracesIcon : TerminalIcon;
  return (
    <div className="flex w-full min-w-0 items-center gap-1.5 overflow-hidden rounded-lg border border-input p-2.5">
      ...
      {!item.passkeyOnly ? <DropdownMenu><DropdownMenuTrigger render={<Button variant="ghost" size="icon-xs" type="button" aria-label={`${item.title} 的更多操作`} title="更多"><EllipsisVerticalIcon size={14} aria-hidden="true" /></Button>} /><DropdownMenuContent><DropdownMenuItem onClick={onEdit}><PencilIcon />编辑</DropdownMenuItem></DropdownMenuContent></DropdownMenu> : null}
    </div>
  );
}
...
function copyOptions(item: VaultItem): string[] {
  ...
  if (item.type === "支付卡") return ["卡号", "安全码", "PIN"];
