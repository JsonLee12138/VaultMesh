...
| `AT-AGENT-UNLOCK-001` | packaged Tauri + 真实 Codex/OpenCode 的独立 Agent 解锁窗口、窗口内 connection/client scope 切换、主密码/Agent PIN、独立 Agent Touch ID record 与每个 request 单次自动提示、失败后的明确反馈及主密码 Enter/按钮 fallback、desktop/browser/other-client 隔离、同调用继续/超时重试、VaultMesh 软件触发的 Agent lock、disconnect/revoke/system-lock/last-lease cleanup 验收 |
