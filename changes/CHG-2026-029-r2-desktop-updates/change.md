...
| `UPD-012` | `REQ-UPDATE-001`, `REQ-EMAIL-001` | Test/Review 与 experimental desktop package 的已接受 Gmail Desktop OAuth Client 绑定、credential pair Provider preflight、单一 Gmail scope 与实际授予 scope fail-closed 门禁 | `CT-UPDATE-001`, `CT-EMAIL-001` | Implementing；Client 前缀与 Provider pair 双门禁及 `.7` hosted/static binding Pass；`.7` live AT 发现 granular consent 未授予 `gmail.readonly`，单一 scope/Profile 修复 regression Pass，`.8` hosted/live retest Pending |
| `UPD-013` | `REQ-UPDATE-001` | Rust 1.95.0 固定工具链、忽略 workspace-only 版本变化的 dependency-only Cargo cache，以及保存前移除可能含编译期 credential 的 workspace release object | `CT-UPDATE-001` | Implemented；cache-key/security/workflow contract Pass；新 cache hosted hit Pending |
...
- 2026-08-10 live Provider correction：`0.0.5-review` 的 Gmail authorization-code exchange 返回 HTTP 400；Google Console 截图确认目标 Client 类型为 Desktop，当时 binary 侧只确认格式有效的 Google Client ID 与 Provider 接受的 credential pair，未证明 exact client 一致。无 Client Secret 的 token-endpoint probe 返回 `client_secret is missing`，加入配套 secret 后进入预期 malformed-code 分支，证明 `.5` 缺少配套 Client Secret。共用构建门禁和四个 package workflow 改为注入并预检 credential pair，使用固定无效授权码且不接触账号或 token；拒绝未知/已删除/不匹配凭据、网络失败和无效响应。本机真实 credential pair Provider gate、Email OTP tests 12/12、完整 Tauri Rust lib 221 pass/1 ignored、lib Clippy、scripts 98/98、Tauri typecheck、docs check 与 diff check Pass；GitHub Actions repository 中两个 credential Secret 名称均已确认存在且未读取其值。
- 2026-08-10 `.6` Gmail live correction：run `31397283749` 的 credential-pair Provider probe 与三平台 package/R2/Draft 均 Pass，但 Gmail API 返回 HTTP 403。目标 Google 项目的 Gmail API、`gmail.readonly` 与测试用户配置均已只读确认；`.6` 与 `.5` binary 的 Client ID 相同但不匹配已接受 `VaultMesh Desktop` 截图前缀，证明 CI 使用了属于其他项目的有效 credential pair。本地运行时环境变量优先于 build-time 值，解释了本地正确变量可以正常使用。GitHub Secrets 已更新；门禁新增已接受 Client 前缀校验并继续执行 Provider pair probe，`.6` immutable artifact 不覆盖，`.7` hosted rebuild Pending。
- 2026-08-10 `.7` hosted rebuild：GitHub Actions [run `31402083758`](https://github.com/atlantis-mk/VaultMesh/actions/runs/31402083758)（source `0b64d18`）的三平台已接受 Client 前缀门禁、credential-pair Provider probe、原生 package、R2 latest-last publish 与六资产 Draft 全部 Pass；公开 Review manifest 为 `0.0.7-review`，恰含三个平台与非空签名。Draft `untagged-590266da3cb1aef7631a` 的 target source 与 run 一致。本机下载 Intel DMG 后只读验证 Mach-O x86_64、`.app` deep/strict code signature，并对原始二进制做非输出布尔匹配确认内嵌已接受 Client 前缀；真实 Gmail 授权与读取 retest Pending，不把该静态证据计为平台 AT。
- 2026-08-11 `.7` live Gmail scope correction：安装态 `.7` 与已接受 Client 绑定正确，Google Cloud 的 Gmail API、restricted scope、Testing audience 与测试用户均正确；loopback callback 的非秘密 scope 集合布尔检查证明只授予身份 scope、没有 `gmail.readonly`。runtime 因未检查实际授予 scope 而保存部分授权，首次 Gmail API 读取返回 HTTP 403。修复只请求 `gmail.readonly`、在账户持久化前拒绝缺失 scope，并通过 Gmail Profile 获取邮箱地址；定向 Rust tests 14/14 Pass，`.8` hosted/live retest Pending。
...
- 2026-08-10 cache v2 安全与复用优化：`.5` 的完整 target cache 每个平台约 438–489 MB，且 `.5 → .6` 只推进 workspace 版本仍产生三组新 key；`.6` run `31397283749` 通过 v1 prefix fallback 把 ARM64、Intel、Windows 从 12m03s/23m06s/23m36s 降至 9m16s/21m48s/21m58s，三平台、R2 publish 与六资产 Draft 全部 Pass，但仍为 `.6` 保存新版本 target cache。Test/Review workflow 现固定 Rust 1.95.0，cache key 忽略 workspace-only SemVer、仍绑定依赖图/manifests/toolchain，并只保存 Cargo downloads 与第三方 dependency objects；artifact 上传后、cache 保存前对四个 workspace package 执行 target-scoped release clean，避免最终包或可能含编译期 OAuth 值的 workspace object 进入 Actions cache。v2 不回退读取旧 v1 target cache；本地 contract tests Pass，首次 hosted v2 hit Pending。
