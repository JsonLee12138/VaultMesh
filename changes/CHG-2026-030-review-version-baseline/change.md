...
| `REV-017` | `REQ-UPDATE-002`, `REQ-BROWSER-004` | `.9` 同 source Chrome/Firefox ZIP 进入不可变 R2 路径并提供公开直接下载 | `CT-UPDATE-REVIEW-001`, `CT-BROWSER-PACKAGE-001` | Done；run `31922859908` 三平台、R2 双 ZIP 公网校验与六资产 Draft Pass |
...
- 2026-08-16 `.9` Review 发布：GitHub Actions [run `31922859908`](https://github.com/atlantis-mk/VaultMesh/actions/runs/31922859908)（source `57ac9f6b7dcc9109c3d5608fc2065c5259dbfbdc`）的 Chrome/Firefox extension、macOS ARM64、macOS Intel、Windows x64、R2 publisher 与 GitHub Draft 全部 Success。公开 Review manifest 为 `0.0.9-review`，仍恰含三个 signed platform URL；R2 同版本不可变路径额外公开两个 extension ZIP，并在 publisher 内从公网下载后与构建产物逐字节一致。Draft `untagged-0ccc8dbffbd57af87705` 为 Draft + Prerelease，六个资产全部 uploaded；远端 `v0.0.9-review` Git Tag 不存在，符合 Review 非正式发布边界。
