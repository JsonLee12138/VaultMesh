...
const repositoryUrl = "git+https://github.com/atlantis-mk/VaultMesh.git";
const canonicalLicenseSha256 =
...
test("CT-LICENSE-001 keeps the canonical PolyForm Noncommercial text", async () => {
  ...
  assert.equal(digest, canonicalLicenseSha256);
...
test("CT-LICENSE-001 keeps pnpm workspace metadata consistent", async () => {
  ...
  for (const manifestPath of manifests) {
    ...
    assert.equal(
      ...
      repositoryUrl,
      `${manifestPath} repository mismatch`,
    );
  }
});

test("CT-LICENSE-001 keeps Rust workspace metadata consistent", async () => {
  const workspace = await read("Cargo.toml");
  assert.match(workspace, /^license = "PolyForm-Noncommercial-1\.0\.0"$/m);
  assert.match(
    workspace,
    /^repository = "https:\/\/github\.com\/atlantis-mk\/VaultMesh"$/m,
  );

  const memberManifests = [
    "crates/agent-mcp/Cargo.toml",
    "crates/vault-core/Cargo.toml",
    "crates/vault-ffi/Cargo.toml",
    "apps/tauri-desktop/src-tauri/Cargo.toml",
  ];

  for (const manifestPath of memberManifests) {
    const manifest = await read(manifestPath);
    assert.match(manifest, /^license\.workspace = true$/m, manifestPath);
    assert.match(manifest, /^repository\.workspace = true$/m, manifestPath);
  }
});
...
test("CT-LICENSE-001 keeps licensing boundaries explicit", async () => {
  ...
  assert.match(readme, /源码可见/);
  assert.doesNotMatch(readme, /开源密码管理器/);
