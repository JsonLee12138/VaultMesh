...
function fixture(status = 'Verified') {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'vaultmesh-work-archive-'));
...
test('rejects rehashing an existing archive relative to the Git baseline', () => {
  ...
  execFileSync('git', ['config', 'user.email', 'archive-test@vaultmesh.invalid'], { cwd: root });
  execFileSync('git', ['config', 'user.name', 'VaultMesh Archive Test'], { cwd: root });
...
test('allows the initial archive registry when the trusted baseline predates it', () => {
  const { root, workId, change } = fixture();
  execFileSync('git', ['init'], { cwd: root, stdio: 'ignore' });
  execFileSync('git', ['config', 'user.email', 'archive-test@vaultmesh.invalid'], { cwd: root });
  execFileSync('git', ['config', 'user.name', 'VaultMesh Archive Test'], { cwd: root });
